...
const char* scoreMetricToStr(enum ScoreMetric metric) {
	if(metric == ILR) {
		return "ILR";
	}
	else if(metric == CLR){
...
//----------------------------------------------------------------------
double*** buildValidBgscore(bool **isValidSite[SPAN_CAPACITY], double ***bgscore, int minspan, int maxspan, Dataset *data) {
	double*** validBgscore;
	...
	if(DEBUG1) {
		for(int m = minspan; m <= maxspan; m++) {
			...
			//printing bgscore
			fprintf(stderr, "\n");
			fprintf(stderr, "validBgscore\n");
			for(int i = 0; i < data->numseqs; i++) { //takeout
				fprintf(stderr, "seqlen[%d] %d\n", i, data->seqlen[i]);
				for(int j= 0; j < data->seqlen[i]; j++) {
					fprintf(stderr, "%d(%.2lg) ", j, validBgscore[span][i][j]);
				}
				fprintf(stderr, "\n");
			}
		}
