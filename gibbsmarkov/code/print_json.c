...
static
void printJsonGapPos(FILE *fptr, Profile *profile ) {
	fprintf(fptr, "\"gap_positions\" : [");
	int gapCount = 0;
...
static
void printKmers(FILE *fptr, int *sites, int span, Dataset *data ) {
	fprintf(fptr, "\"kmers\" : {");
	...
	for(int i = 0; i < data->numseqs; i++) {
		if(sites[i] >= 0) {
			...
			fprintf(fptr, "\"%d\" : [%d, \"", i, abs(getConcat2DoubleStrandPos(data, i, sites[i])) );
			for(int j = 0; j < span; j++) {
				...
				int pos = j + sites[i];
				if(sites[i] <= pos && pos < sites[i] + span) {
					fprintf(fptr, "%c", numToChar(data->seqs[i][pos])); //upper case
				}
			}
			fprintf(fptr, "\", %d, \"%c\"]", 
					abs(getConcat2DoubleStrandPos(data, i, sites[i] + span - 1)), 
					isForwardStrand(data, i, sites[i]) ? '+' : '-');
		}
	}
	fprintf(fptr, "},\n");
}
...
static
void printJsonCountmatAndSites(FILE *fptr, Profile *countmat, int *sites, double *scorePerSite, Dataset *data)
{
	...
	//printJsonProfile(fptr, profile);
	//printJsonFlankingRegionProfile(fptr, sites, countmat->span, data);

	fprintf(fptr, "\"motif_span\" : %d,\n", profile->span);
	fprintf(fptr, "\"motif_weight\" : %d,\n", profile->cols);

	fprintf(fptr, "\"consensus_motif\" : \"");
	...
	fprintf(fptr, "\"num_sites\" : %d,\n", numsites);
	fprintf(fptr, "\"num_seqs\" : %d,\n", data->numseqs);

	if(profile->span != profile->cols) {
		printJsonGapPos(fptr, profile);
	}
	printKmers(fptr, sites, profile->span, data);

	//Sequence range is from 0 to (number of sequences - 1)
	//Motif sites are in range 1 to length or -1 to -length
	fprintf(fptr, "\"sites\" : {");
	bool isFirstSite = true;
	for(int i=0; i< data->numseqs; i++) {
		if(sites[i] >= 0) {
			...
			fprintf(fptr, "\"%d\": [%d, %d]",i, pos, revcomplPos);
		}
	}
	fprintf(fptr, "}\n");
...
void printJsonRunNode(FILE *fptr, RunNode *node, enum ScoreMetric metric, Markov *markov, Dataset *data, Zoops *zoops) {
	if(DEBUG0) {
	...
	if(metric == ILR) {
		...
		fprintf(fptr, "\"log_markovian_ilr_pwm\" : %.5lf,\n", ilr_pwm);
	...
	fprintf(fptr, "\"log_markovian_ilr_sites_without_pseudocount\" : %.5lf,\n", ilr_sites);

	//CLR from sites
	double entscore= computeEntropy(markov, data, zoops, node->sites, node->countmat->span);
	fprintf(fptr, "\"log_markovian_clr_sites_without_pseudocount\" : %.5lf,\n", entscore);

	fprintf(fptr, "\"log_clr_normalized_by_numseqs\" : %.5lf,\n", entscore / data->numseqs);

	//Score that was used as the best run
	fprintf(fptr, "\"score_ranking_runs\" : %.5lf,\n", node->score);
	...
	printJsonCountmatAndSites(fptr, node->countmat, node->sites, scorePerSite, data);
	fprintf(fptr, "}\n");
