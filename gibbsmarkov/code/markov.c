...
double getTransProb(
		...
		Dataset *data,
		double **posNullprob,
		int *minBeginDependIndArr,
		int seqind,
		int siteind,
		int beginDependInd
){
	if(beginDependInd < minBeginDependIndArr[siteind]) { //take max()
		beginDependInd = minBeginDependIndArr[siteind];
	}
	...
	return posNullprob[order][siteind];
...
//--------------------------------------------------------------------------------------------
static
int getActualMarkovOrder(int *minBeginDependIndArr, int pos, int ord) {
	int j = pos;
	...
	if(ord > j - minBeginDependIndArr[j]) { //take min()
		actualOrder = j - minBeginDependIndArr[j];
	}
	else {
		actualOrder = ord;
	}
	if(DEBUG0) {
		if(actualOrder < 0) {
			fprintf(stderr, "minBeginDependIndArr[%d] = %d\n", j, minBeginDependIndArr[j]);
			fprintf(stderr, "FATAL ERROR: site %d seems to be a bad character at getActualMarkovOrder(). actualOrder: %d\n",
					j, actualOrder
			);
			exit(1);
		}
...
int getActualMarkovOrder(int **minBeginDependInd, int seqind, int pos, int ord) {
	int actualOrder = getActualMarkovOrder(minBeginDependInd[seqind], pos, ord);
	return actualOrder;
}

static
void precomputePosNullprob(
		double **posNullprob, //[ord][seqlen]
		int seqind,
		int *minBeginDependIndArr, //for current sequence seqind
		TransProb **seqtransprob,
		Markov *markov,
		Dataset *data
) {
	TransProb *transprob = seqtransprob[seqind];
	for(int ord = 0; ord <= markov->maxorder; ord++) {
		for(int j = 0; j < markov->seqlen[seqind]; j++) {
			if(data->isBadPos[seqind][j]) {
				posNullprob[ord][j] = 0.0;
			}
			else {
				int actualOrder = getActualMarkovOrder(minBeginDependIndArr, j, ord);

				//determine the previous alphabets of the word
				int alphas[MARKOV_ORDER_BOUND + 1];
				for(int a = 0; a <= actualOrder; a++) {
					alphas[a] = data->seqs[seqind][j - a];
				}

				//compute transition probability
				if(actualOrder == 0) {
					posNullprob[ord][j] = getTransProb(transprob, actualOrder, alphas[0], -1, -1, -1, -1, -1);
				}
				else if(actualOrder == 1) {
					posNullprob[ord][j] = getTransProb(transprob, actualOrder, alphas[0], alphas[1], -1, -1, -1, -1);
				}
				else if(actualOrder == 2) {
					posNullprob[ord][j] = getTransProb(transprob, actualOrder, alphas[0], alphas[1], alphas[2], -1, -1, -1);
				}
				else if(actualOrder == 3) {
					posNullprob[ord][j] = getTransProb(transprob, actualOrder, alphas[0], alphas[1], alphas[2], alphas[3], -1, -1);
				}
				else if(actualOrder == 4) {
					posNullprob[ord][j] = getTransProb(transprob, actualOrder, alphas[0], alphas[1], alphas[2], alphas[3], alphas[4], -1);
				}
				else if(actualOrder == 5) {
					posNullprob[ord][j] = getTransProb(transprob, actualOrder, alphas[0], alphas[1], alphas[2], alphas[3], alphas[4], alphas[5]);
				}
				else {
...
static
void computeMinBeginDependIndArr(Dataset *data, int *minBeginDependIndArr, int seqind) {
	...
	if(data->useRevcompl) {
		int back_start = data->seqlen[seqind]/2; //starting position for backward strand
		//forward strand
		for(int j = 0; j < back_start; j++) {
			minBeginDependIndArr[j] = 0;
		}
		for(int j = back_start; j < data->seqlen[seqind]; j++) {
			minBeginDependIndArr[j] = back_start;
		}
	}
	else {
		for(int j = 0; j < data->seqlen[seqind]; j++) {
			minBeginDependIndArr[j] = 0;
		}
	}
	...
	for(int j = 0; j < data->seqlen[seqind]; j++) {
		...
		if(minBeginDependIndArr[j] < index) {
			minBeginDependIndArr[j] = index;
		}
	}
}

static
void precomputeBgscorePerSeq(
		double **posNullprob, //[ord][seqlen]
		int seqind,
		int *minBeginDependIndArr,
		Markov *markov,
	...
	for(int j= 0; j < data->seqlen[seqind]; j++) {
		score[seqind][j] = NAN; //for out-of-bound (j >= seqlen[i] - span + 1)
	}

	//precompute
	if(markov->bgmodel == BG_GIBBSMARKOV) {
		for(int j= 0; j < data->seqlen[seqind] - span + 1; j++) {
			if(!data->isValidSite[span][seqind][j]) {
				score[seqind][j] = PINF;
			}
			else {
				score[seqind][j] = 1.0;
				for(int m = 0; m < span; m++) {
					score[seqind][j] *= getTransProb(markov,data,posNullprob, minBeginDependIndArr, seqind, j+m, 0);
				}
				for(int k = 0; k < markov->maxorder; k++) { //exception from using "<= maxorder". See DEBUG0 check below.
					...
					if(j+span+k < data->seqlen[seqind] && !data->isBadPos[seqind][j+span+k]) {
						score[seqind][j] *= getTransProb(markov,data,posNullprob, minBeginDependIndArr, seqind, j+span+k, 0)
										/ getTransProb(markov, data,posNullprob, minBeginDependIndArr, seqind, j+span+k, j+span);
					}
					else {
						break;
					}
				...
				if(DEBUG0) {
					...
					if(j+span+k < data->seqlen[seqind] && !data->isBadPos[seqind][j+span+k] ) {
						double ratio = getTransProb(markov,data,posNullprob, minBeginDependIndArr, seqind, j+span+k, 0)
										/ getTransProb(markov, data, posNullprob, minBeginDependIndArr, seqind, j+span+k, j+span);
		...
		for(int j= 0; j < data->seqlen[seqind] - span + 1; j++) {
			if(!data->isValidSite[span][seqind][j]) {
				...
				score[seqind][j] = 1.0;
				for(int m = 0; m < span; m++) {
					score[seqind][j] *= getTransProb(markov,data, posNullprob, minBeginDependIndArr, seqind, j+m, j);
				}
			}
		}
	}
		...
		for(int j= 0; j < data->seqlen[seqind] - span + 1; j++) {
			if(!data->isValidSite[span][seqind][j]) {
				...
				score[seqind][j] = 1.0;
				for(int m = 0; m < span; m++) {
					double score1 = getTransProb(markov,data, posNullprob, minBeginDependIndArr, seqind, j+m, 0);
					double score2 = getTransProb(markov, data, posNullprob, minBeginDependIndArr, seqind, getConcatPosOfOppStrand(data, seqind, j+m, 1), 0);

					score[seqind][j] *= sqrt(score1 * score2);
				}
			}
		}
	}
		...
		for(int j= 0; j < data->seqlen[seqind] - span + 1; j++) {
			if(!data->isValidSite[span][seqind][j]) {
				...
				for(int m = 0; m < span; m++) {
					...
					score[seqind][j] *= (score1 + score2) / 2.0;
				}
			}
		}
	}
...
//	}
	...
	double **posNullprob = (double**) malloc(sizeof(double*) * (markov->maxorder+1));
	for(int ord = 0; ord <= markov->maxorder; ord++) {
		posNullprob[ord] = (double*) malloc(sizeof(double) * data->maxseqlen);
	}

//	int **minBeginDependInd = (int**) malloc(sizeof(int*) * data->numseqs);
//	for(int i=0; i < data->numseqs; i++) {
//		minBeginDependInd[i] = (int*) malloc(sizeof(int) * data->seqlen[i]);
//	}
	int *minBeginDependIndArr = (int*) malloc(sizeof(int) * data->maxseqlen);
...
//			fprintf(stderr, "seqlen[%d] %d\n", i, data->seqlen[i]);
//			for(int j= 0; j < data->seqlen[i]; j++) {
//				fprintf(stderr, "%d(%d) ", j, minBeginDependInd[i][j]);
//			}
//			fprintf(stderr, "\n");
//		}
//		fprintf(stderr, "\n");
...
//	}
	...
	for(int i = 0; i < data->numseqs; i++) { //takeout
		if(DEBUG1) {
			fprintf(stderr, "Building markov background matrix for sequence %d\n", i);
		}
		computeMinBeginDependIndArr(data, minBeginDependIndArr, i);

		precomputePosNullprob(
				posNullprob,
				i,
				minBeginDependIndArr,
				seqtransprob,
				markov,
				data
		);
		...
		for(int m = minspan; m <= maxspan; m++) {
			//precomputeBgscore separated by each sequence
			precomputeBgscorePerSeq(posNullprob, i, minBeginDependIndArr, markov, data, m);
		}
	}
	...
	//deallocate minBeginDependInd
	free(minBeginDependIndArr);

	//deallocate posNullprob
	for(int ord = 0; ord <= markov->maxorder; ord++) {
		free(posNullprob[ord]);
	}
	free(posNullprob);
