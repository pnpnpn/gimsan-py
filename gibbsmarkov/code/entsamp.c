	...
	for(int i = 0; i < gibbs->data->numseqs; i++) {
		...
		if(gibbs->data->numValidSites[countmat->span][i] == 0) {
			...
			for(int j = 0; j < gibbs->data->seqlen[i] - countmat->span + 1; j++) {
				...
				if(DEBUG1) {
					if(validBgscore[j] * fgScore > 1.0) {
						fprintf(stderr, "#");
					}
					fprintf(stderr, "i=%-3d j=%-5d bg=%-10.3lg f=%-10.3lg\n", i,j, validBgscore[j], fgScore);
				}
			}
...
void initGibbsStructs(Gibbs *gibbs) {
	...
	if(DEBUG0) {
		fprintf(stderr, "Estimating Markov model ...\n");
	}
	...
	if(gibbs->pspfilename != NULL) {
		if(DEBUG0) {
			fprintf(stderr, "Estimating PSP model ...\n");
		}
	...
	if(DEBUG0) {
		fprintf(stderr, "Building valid bgscore ...\n");
	}
