...
static
void printCountmatAndSites(FILE *fptr, Profile *countmat, int *sites, double *scorePerSite, Dataset *data)
{
	...
	/* print profile */
	fprintf(fptr, "Motif profile generated from sites:\n"); 
	printProfile(fptr, profile);

	fprintf(fptr, "Flanking regions:\n"); 
	printFlankingRegionProfile(fptr, sites, countmat->span, data);


	/* NOW, PRINT CONSENSUS MOTIF */
	fprintf(fptr, "Motif span: %d\n", profile->span);
	fprintf(fptr, "Motif weight/width: %d\n", profile->cols);
	fprintf(fptr, "Consensus motif: ");
	...
	fprintf(fptr, "Number of predicted sites: %d (%.2lf%%)\n", numsites, (numsites * 100.0) / data->numseqs);
	fprintf(fptr, "\n");

	if(profile->span != profile->cols) {
		printGapPos(fptr, profile);
		fprintf(fptr, "\n");
	}

	fprintf(fptr, "Sequence range is from 0 to (number of sequences - 1).\n");
	fprintf(fptr, "Motif sites are in range 1 to length or -1 to -length.\n");
	for(int i=0; i< data->numseqs; i++) {
		if(sites[i] >= 0) {
			...
			fprintf(fptr, "Sequence %3d: motif site %6d  (%6d)\n",i, pos, revcomplPos);
		}
		else {
			fprintf(fptr, "Sequence %3d: motif site %6s  (%6s)\n",i, "none", "none");
		}
	}
	

	fprintf(fptr, "\n");
	fprintf(fptr, "Alignments and FASTA-headers:\n");
	printAlignment(fptr, sites, profile->span, scorePerSite, data);
	fprintf(fptr, "\n");
	if(numsites < data->numseqs) {
		fprintf(fptr, "FASTA-header of sequences with zero occurrences:\n");
		printZeroOccurrencesSeqHeaders(fptr, sites, data);
		fprintf(fptr, "\n");
	}
	nilProfile(profile);
}

void printRunNode(FILE *fptr, RunNode *node, enum ScoreMetric metric, Markov *markov, Dataset *data, Zoops *zoops) {
	if(DEBUG0) {
		assert(node->countmat->span == node->pswm->span);
	}

	fprintf(fptr, "All of the following scores are computed after EM (if applicable).\n");
	if(metric == ILR) {
		//ILR from PWM
		double ilr_pwm = computeIlr(markov, data, node->pswm->mat, node->pswm->span);
		fprintf(fptr, "Log Markovian-ILR of PWM: %.2lf\n", ilr_pwm);
	...
	fprintf(fptr,"Log Markovian-ILR generated from sites without pseudocount: %.2lf\n", ilr_sites);

	//CLR from sites
	double entscore= computeEntropy(markov, data, zoops, node->sites, node->countmat->span);
	fprintf(fptr, "Log Markovian-CLR generated from sites without pseudocount: %.2lf\n", entscore);

	fprintf(fptr, "Log(CLR) normalized by number of sequences: %.4lf\n", entscore / data->numseqs);

	//Score that was used as the best run
	fprintf(fptr, "Score for ranking runs: %.2lf\n", node->score);
	fprintf(fptr, "\n");

	//printf("PWM after EM:\n");
	//printProfile(fptr, node->pswm); //hopefully, no one screwed around with this matrix
	//printf("\n");
	...
	printCountmatAndSites(fptr, node->countmat, node->sites, scorePerSite, data);
